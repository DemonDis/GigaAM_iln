...
def calculate_rouge_n(reference_tokens: List[str], hypothesis_tokens: List[str], n: int) -> Dict[str, float]:
    ...
    recall = overlap / sum(ref_ngrams.values()) if sum(ref_ngrams.values()) else 0.0
...
def create_app(model=None, device=None):
    ...
    @app.get("/device")
    def get_device():
        return {"device": str(device) if device is not None else "cpu"}
    ...
    async def quantitative_metrics(request: MetricsRequest):
        ...
        return {
            ...
            "rouge": {"rouge1": rouge1, "rouge2": rouge2, "rougeL": rouge_l},
        }

    @app.post("/export/md")
    async def export_md(request: ExportRequest):
        return {"content": request.protocol, "filename": get_filename("Протокол", "md")}
    ...
    async def export_docx(request: ExportRequest):
        ...
        doc.save(buffer)
        buffer.seek(0)
        return {"content": buffer.getvalue().decode('latin-1'), "filename": get_filename("Протокол", "docx"), "content_type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document"}
